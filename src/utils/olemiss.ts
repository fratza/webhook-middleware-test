...
export function parseEventDate(eventDateStr: string): { startDate: string; endDate: string } | null {
    ...
    const match = eventDateStr.match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{1,2})(?:-(\d{1,2}))?,\s*(\d{4})/);
 ...
 */
export function processOleMissItem(processedItem: any, newLabel: any, key: string, docName: string, originUrl: string): any {
    // Check if Score_date field contains HTML content
    if (
        processedItem.Score_date &&
        typeof processedItem.Score_date === 'string' &&
        processedItem.Score_date.includes('s-game-card-standard__header')
    ) {
        const gameDetails = extractGameDetails(processedItem.Score_date);
